...
public class ShiftDemandResource {
     ...
     */
    @GetMapping("")
    public List<ShiftDemand> getAllShiftDemands(@RequestParam(name = "filter", required = false) String filter) {
        if ("department-is-null".equals(filter)) {
            log.debug("REST request to get all ShiftDemands where department is null");
            return StreamSupport
                .stream(shiftDemandRepository.findAll().spliterator(), false)
                .filter(shiftDemand -> shiftDemand.getDepartment() == null)
                .toList();
        }

        if ("shift-is-null".equals(filter)) {
            log.debug("REST request to get all ShiftDemands where shift is null");
            return StreamSupport
                .stream(shiftDemandRepository.findAll().spliterator(), false)
                .filter(shiftDemand -> shiftDemand.getShift() == null)
                .toList();
        }
