...
public class ResourceTrainingResource {
     ...
     */
    @GetMapping("")
    public List<ResourceTraining> getAllResourceTrainings(@RequestParam(name = "filter", required = false) String filter) {
        if ("resource-is-null".equals(filter)) {
            log.debug("REST request to get all ResourceTrainings where resource is null");
            return StreamSupport
                .stream(resourceTrainingRepository.findAll().spliterator(), false)
                .filter(resourceTraining -> resourceTraining.getResource() == null)
                .toList();
        }

        if ("training-is-null".equals(filter)) {
            log.debug("REST request to get all ResourceTrainings where training is null");
            return StreamSupport
                .stream(resourceTrainingRepository.findAll().spliterator(), false)
                .filter(resourceTraining -> resourceTraining.getTraining() == null)
                .toList();
        }
