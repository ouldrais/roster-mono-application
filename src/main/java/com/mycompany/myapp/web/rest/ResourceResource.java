...
public class ResourceResource {
     ...
     */
    @GetMapping("")
    public List<Resource> getAllResources(@RequestParam(name = "filter", required = false) String filter) {
        if ("team-is-null".equals(filter)) {
            log.debug("REST request to get all Resources where team is null");
            return StreamSupport
                .stream(resourceRepository.findAll().spliterator(), false)
                .filter(resource -> resource.getTeam() == null)
                .toList();
        }
