...
public class PositionRequirementResource {
     ...
     */
    @GetMapping("")
    public List<PositionRequirement> getAllPositionRequirements(@RequestParam(name = "filter", required = false) String filter) {
        if ("training-is-null".equals(filter)) {
            log.debug("REST request to get all PositionRequirements where training is null");
            return StreamSupport
                .stream(positionRequirementRepository.findAll().spliterator(), false)
                .filter(positionRequirement -> positionRequirement.getTraining() == null)
                .toList();
        }

        if ("position-is-null".equals(filter)) {
            log.debug("REST request to get all PositionRequirements where position is null");
            return StreamSupport
                .stream(positionRequirementRepository.findAll().spliterator(), false)
                .filter(positionRequirement -> positionRequirement.getPosition() == null)
                .toList();
        }
