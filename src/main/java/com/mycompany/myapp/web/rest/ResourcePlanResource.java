...
public class ResourcePlanResource {
     ...
     */
    @GetMapping("")
    public List<ResourcePlan> getAllResourcePlans(@RequestParam(name = "filter", required = false) String filter) {
        if ("resource-is-null".equals(filter)) {
            log.debug("REST request to get all ResourcePlans where resource is null");
            return StreamSupport
                .stream(resourcePlanRepository.findAll().spliterator(), false)
                .filter(resourcePlan -> resourcePlan.getResource() == null)
                .toList();
        }

        if ("position-is-null".equals(filter)) {
            log.debug("REST request to get all ResourcePlans where position is null");
            return StreamSupport
                .stream(resourcePlanRepository.findAll().spliterator(), false)
                .filter(resourcePlan -> resourcePlan.getPosition() == null)
                .toList();
        }

        if ("shift-is-null".equals(filter)) {
            log.debug("REST request to get all ResourcePlans where shift is null");
            return StreamSupport
                .stream(resourcePlanRepository.findAll().spliterator(), false)
                .filter(resourcePlan -> resourcePlan.getShift() == null)
                .toList();
        }
