...
public class TeamPlanResource {
     ...
     */
    @GetMapping("")
    public List<TeamPlan> getAllTeamPlans(@RequestParam(name = "filter", required = false) String filter) {
        if ("team-is-null".equals(filter)) {
            log.debug("REST request to get all TeamPlans where team is null");
            return StreamSupport
                .stream(teamPlanRepository.findAll().spliterator(), false)
                .filter(teamPlan -> teamPlan.getTeam() == null)
                .toList();
        }

        if ("shift-is-null".equals(filter)) {
            log.debug("REST request to get all TeamPlans where shift is null");
            return StreamSupport
                .stream(teamPlanRepository.findAll().spliterator(), false)
                .filter(teamPlan -> teamPlan.getShift() == null)
                .toList();
        }
